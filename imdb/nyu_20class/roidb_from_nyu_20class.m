function roidb = roidb_from_nyu_20class(imdb)
...
cache_file = ['./imdb/nyu_20class/cache/roidb_' imdb.name];
try
  load(cache_file);
catch
...
%        class: [2108x1 uint8]
%% Remove the objects of class 'None'
%% I had to use it because PASCAL doens't allow any
%% file without any object, while this expects every
%% object to be in the list
for obid = 1 : numel(voc_rec.objects)
  if strcmp(voc_rec.objects(obid).class, 'None') == 1
    voc_rec.objects(obid) = [];
  end
end
